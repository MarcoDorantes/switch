  ...
  public class Switch : IEnumerable<string>
  {
    ...
    public static SchemaType AsType<SchemaType>(Switch opts, SchemaType target)
    {
      ...
      foreach (KeyValuePair<string, string> named in opts.NamedArguments)
      {
        ...
        ParseAndAssignValue(schema, target, opts, member_type, member_name);
      }
    ...
    public TargetType AsType<TargetType>(int index)
    {
      ...
      return (TargetType)ParseTypeValue(this, this[index], typeof(TargetType), index, default(TargetType), itemprovider);
    }

    public TargetType AsType<TargetType>(string opt_name)
    {
      return (TargetType)ParseTypeMemberValue(this, typeof(TargetType), opt_name, default(TargetType));
    }

    private static void ParseAndAssignValue(Type schema, object target, Switch opts, Type member_type, string member_name)
    {
      ...
      object value = ParseTypeMemberValue(opts, member_type, member_name, default_value);
    ...
    private static object ParseTypeMemberValue(Switch opts, Type member_type, string member_name, object default_value)
    {
      if (member_type == typeof(bool))
        return opts.Is(member_name);
      Function<Switch, string, IList<string>> itemprovider = (args, name) => new List<string>(args.SuffixOf(name));
      return ParseTypeValue(opts, opts[member_name], member_type, member_name, default_value, itemprovider);
    }

    private static object ParseTypeValue<T>(Switch opts, string typevalue, Type member_type, T member_name_or_index, object default_value, Function<Switch, T, IList<string>> itemsprovider)
    {
      ...
      if (IsDictionary(member_type))
        return ParseDictionary(opts, member_type, member_name_or_index, default_value);
    ...
    private static object ParseDictionary<T>(Switch opts, Type member_type, T member_name_or_index, object default_value)
    {
      ...
      foreach (KeyValuePair<string, string> keyvaluepair in sub_opts.NamedArguments)
        result.Add(ParseTypeLiteralValue(keytype, keyvaluepair.Key, key_default_value), ParseTypeValue<string>(sub_opts, keyvaluepair.Value, valuetype, keyvaluepair.Key, value_default_value, itemprovider));
      return result;
    }
